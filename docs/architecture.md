...
|   Lidar Process, |      |    RLCarController,    |      |      & Types Contract)|
|   Lidar Simulator|      |    SimBackend)         |      +-----------------------+
+------------------+      +------------------------+                  ^
         ^                             ^                              |
         | Raycasts                    | Commands                     |
         v                             v                              |
...
* **`src/perception/` (Sensors & CV)**: Evaluates camera frames for lanes, processes Lidar signals for obstacle ranges, and implements the virtual `LidarSimulator` queries.
...
1. **Sensor Measurement**: The `RLCarController` queries `LidarSimulator` (perception) to obtain current distance ranges.
2. **Observation Construction**: Telemetry metrics (speed) and Lidar ranges are combined to form the state observation vector.
